...
public class CacheB<T> {
	
	private final static Map<Class<?>, CacheB<?>> caches = new ConcurrentHashMap<>();
	
	public static <T> void allocateCache(Class<T> clazz, Queue<T> queue) {
		if(!caches.containsKey(clazz)) caches.put(clazz, new CacheB<>(queue, clazz));
	}
	
	public static <T> void allocateCache(Class<T> clazz, Queue<T> queue, Supplier<T> supplier) {
		if(!caches.containsKey(clazz)) caches.put(clazz, new CacheB<>(queue, supplier));
	}
	
	public static <T> T get(Class<T> clazz) {
		CacheB<?> cache = caches.get(clazz);
		if(cache == null) return get0(clazz);
		return clazz.cast(cache.get());
	}
	
	public static void cache(Object obj) {
		caches.computeIfAbsent(obj.getClass(), CacheB::new).add(obj);
	}
	...
	private CacheB(Class<T> clazz) {
		this(new ConcurrentLinkedQueue<>(), clazz);
	}
	
	private CacheB(Queue<T> cache, Class<T> clazz) {
		this(cache, () -> {
			try {
				return clazz.newInstance();
	...
	private CacheB(Queue<T> cache, Supplier<T> supplier) {
		this.cache = cache;
